...
// Function to update the download button visibility based on the toggle state
function updateDownloadButtonVisibility() {
  const downloadButton = document.getElementById("download1");
  downloadButton.style.visibility = toggleDownloadButton.checked ? "visible" : "hidden";
}

// Update the download button visibility on initial load
updateDownloadButtonVisibility();

// Event listener to handle toggle changes for download button
toggleDownloadButton.addEventListener("change", function () {
  // Update the download button visibility based on the toggle state
  updateDownloadButtonVisibility();
});

document.getElementById("download1").addEventListener("click", function() {
  // Get the file name from the button's id.
  var fileName = "PlsDonateMap.rbxm";

  // Create a new Blob object with the file data.
  var blob = new Blob(["This is the file data."], {type: "application/octet-stream"});
...
// Disable toggle on mobile and tablet devices
if (isMobileOrTablet()) {
  toggleLoadingScreen.disabled = true;
  toggleDownloadButton.disabled = true;
}